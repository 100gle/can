...
*   **支持格式**:
    ...
    *   PDF (可选，使用 PDF.js)
*   **关键任务**:
    *   [ ] 前端: 实现 `FilePreviewModal` 组件。
    *   [ ] 后端: 确保 `PresignedURL` 生成逻辑支持 `response-content-disposition: inline` 以便浏览器预览。
...
*   **关键任务**:
    *   [ ] 前端: 在预览器中增加 "编辑" 模式。
    *   [ ] 前端: 实现保存逻辑 (获取内容 -> PutObject)。
    *   [ ] UX: 处理并发冲突 (简单的 Last-Write-Wins 或保存前检查 ETag)。

## 3. 对象锁定与合规保留 (Object Lock)
*   **功能描述**: 支持 S3 Object Lock (WORM - Write Once Read Many)，防止对象在固定期限内被删除或篡改。
*   **关联文档**: `docs/features.md` (Item 155)
*   **关键任务**:
    *   [ ] 后端: 适配 `GetObjectLockConfiguration`, `PutObjectRetention`, `PutObjectLegalHold`。
    *   [ ] 前端: 在对象属性侧边栏增加 "Compliance & Retention" 面板。

## 4. 高级元数据与软链接
*   **功能描述**: 完善对象属性的展示，支持识别和创建 Symlink (OSS)。
*   **关联文档**: `docs/features.md` (Item 156)
*   **关键任务**:
    *   [ ] 后端: 在 `HeadObject` 中解析 `x-oss-object-type: Symlink` 等特定头。
    *   [ ] 前端: 在文件列表为软显示特殊图标。
    *   [ ] 前端: 创建软链接的对话框。
