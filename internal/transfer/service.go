...
type DownloadRequest struct {
	AccountID        string `validate:"required"`
	Bucket           string `validate:"required_without=Entries,omitempty,bucket-name"`
	Key              string `validate:"required_without=Entries,omitempty,object-key"`
	SavePath         string `validate:"omitempty"`
	TargetDirectory  string `validate:"omitempty"`
	Priority         Priority
	Mode             DownloadMode
	Entries          []DownloadEntry      `validate:"omitempty,dive"`
	ArchiveName      string               `validate:"omitempty"`
	ConflictStrategy FileConflictStrategy `validate:"omitempty,oneof=overwrite rename"`
	DisableResume    bool
	VersionID        string `validate:"omitempty"`
	ExpectedETag     string `validate:"omitempty"`
}
...
func NewService(accounts *accounts.Service, vault *storage.ClientVault, store Store, opts ...Option) *Service {
	if store == nil {
...
func (s *Service) EnqueueUpload(ctx context.Context, req UploadRequest) (*TransferTask, error) {
	...
	log.Info().
		Str("component", "transfer.EnqueueUpload").
		Str("taskId", task.ID).
		Str("accountId", req.AccountID).
		Str("bucket", req.Bucket).
		Str("key", req.Key).
		Int64("size", task.Total).
		Msg("Enqueued upload task")
...
func (s *Service) EnqueueDownload(ctx context.Context, req DownloadRequest) (*TransferTask, error) {
	...
	log.Info().
		Str("component", "transfer.EnqueueDownload").
		Str("taskId", task.ID).
		Str("accountId", req.AccountID).
		Str("bucket", task.Bucket).
		Str("key", task.Key).
		Interface("mode", task.Type).
		Int64("total", task.Total).
		Msg("Enqueued download task")
	s.enqueue(task.ID, task.Priority)
	return task.clone(), nil
}
...
func (s *Service) executeSingleDownload(ctx context.Context, task *TransferTask) error {
	...
	if offset > 0 {
		input.RangeStart = aws.Int64(offset)
	}
