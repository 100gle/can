...
import { useAccounts, useDial } from "@/hooks/useAccounts";
...
export const ConnectionDialButton = ({ accountId, disabled }: ConnectionTestButtonProps) => {
  const { t } = useTranslation("common");
  const { dialStatus } = useAccounts();
  const dial = useDial();
  const [dialing, setDialing] = useState(false);
  const probe = accountId ? dialStatus[accountId] : undefined;

  const handleDial = async () => {
    if (!accountId) return;
    setDialing(true);
    try {
      await dial.mutateAsync(accountId);
    } catch (error) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 text-sm">
      <Button
        ...
        size="sm"
        className="gap-2"
        disabled={!accountId || disabled || dialing}
        onClick={handleDial}
      >
        {dialing ? (
        ...
        {dialing ? t("account.status.running") : t("account.form.test.button.idle")}
      </Button>
