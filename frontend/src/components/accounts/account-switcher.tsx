...
import { setActiveAccount, useAccounts, useDial } from "@/hooks/useAccounts";
...
export const AccountSwitcher = () => {
  ...
  const { accounts, activeAccountId, dialStatus } = useAccounts();
  const dial = useDial();
  ...
  useEffect(() => {
    ...
    const checkConnection = () => {
      const probe = dialStatus[activeAccountId];
      const now = Date.now();
      ...
      void dial.mutateAsync(activeAccountId).catch(() => undefined);
    };
  ...
  const handleSelect = async (accountId: string) => {
    ...
    void dial.mutateAsync(accountId).catch((error) => {
