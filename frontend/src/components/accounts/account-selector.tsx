...
export const AccountSelector = ({
  ...
  const confirmDelete = async () => {
    ...
    await deleteAccount.mutateAsync(pendingDelete.id).catch(() => {});
    setPendingDelete(null);
  };

  const renderGrid = (layout: "card" | "list") => {
    if (loading) {
      return (
  ...
  return (
    <section>
      ...
      >
        ...
        {accounts.length > 0 && (
          <div className="flex justify-start">
            <TabsList className="flex rounded-lg border border-border/60 bg-muted/20 p-1 text-muted-foreground shadow-inner shadow-black/5 backdrop-blur-sm">
              <TabsTrigger
                value="card"
                aria-label={t("account.selector.view.cardLabel")}
              ...
              >
                ...
                <span className="hidden sm:inline">{t("account.selector.view.card")}</span>
              </TabsTrigger>
              <TabsTrigger
                value="list"
        ...
        <TabsContent value="card">{renderGrid("card")}</TabsContent>
