...
function CreateBucketDialogContent({
  ...
  const createBucketMutation = useCreateBucket(accountId);
  ...
  const bucketNameValidation = useMemo(() => {
    if (!newBucketName.trim()) return { valid: true }; // Empty is ok until submit
    return validateBucketName(provider, newBucketName);
  }, [provider, newBucketName]);

  const bucketNameError = bucketNameValidation.valid ? null : t(bucketNameValidation.error ?? "");
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await createBucketMutation.mutateAsync(payload);
      onClose();
    } catch (e) {
    ...
    <>
      ...
      <DialogFooter className="gap-2">
        ...
        <Button
          ...
          disabled={creating || !newBucketName.trim() || !bucketNameValidation.valid}
        >
