  ...
  return (
    <>
      ...
      <CreateBucketDialog
        ...
        accountId={accountId}
        provider={controller.activeAccount?.provider?.toLowerCase() ?? "aws"}
      ...
      <DestructiveConfirmDialog
        ...
        title={t("explorer.dialog.batchDelete.title")}
        description={t("explorer.dialog.batchDelete.description", {
          count: controller.selectedKeys.size,
        })}
        confirmText={t("actions.delete.confirmText")}
        onConfirm={async () => {
          try {
            const keys = Array.from(controller.selectedKeys);
            await actions.handleDeleteSelected(keys);
          } catch (error) {
            // Error handled in actions
            logger.debug("fileExplorer.deleteSelected", "Batch delete error caught in dialog", {
              error,
            });
          } finally {
      ...
      <DestructiveConfirmDialog
        ...
        title={t("explorer.dialog.deleteBucket.title")}
        description={t("explorer.dialog.deleteBucket.description", {
          name: actions.pendingDeleteBucket,
        })}
        confirmText={t("actions.delete.confirmText")}
        onConfirm={actions.handleDeleteBucket}
      />

      <DestructiveConfirmDialog
        open={!!actions.pendingDeleteObject}
        onOpenChange={(open) => !open && actions.setPendingDeleteObject(null)}
        title={t("explorer.dialog.deleteFile.title")}
        description={t("explorer.dialog.deleteFile.description", {
          name: actions.pendingDeleteObject,
        })}
        confirmText={t("actions.delete.confirmText")}
        onConfirm={actions.handleDeleteObject}
      />
    </>
  );
};
