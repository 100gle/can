...
import { useBucketLifecycle, useUpdateLifecycle } from "@/hooks/useBucketConfig";
...
export const LifecyclePanel = () => {
  ...
  const { data: lifecycle } = useBucketLifecycle(accountId!, bucket!);
  const updateMutation = useUpdateLifecycle();
  ...
  const handleSave = async () => {
    ...
    try {
      await updateMutation.mutateAsync({ accountId, bucket, rules });
    } catch (e) {
