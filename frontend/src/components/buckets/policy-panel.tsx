...
import { useBucketPolicy, useDeletePolicy, useUpdatePolicy } from "@/hooks/useBucketConfig";
...
export const PolicyPanel = () => {
  ...
  const { data: policy, error: fetchError } = useBucketPolicy(accountId!, bucket!);
  ...
  useEffect(() => {
    if (policy?.raw) {
      setJson(policy.raw);
    } else if (policy?.statement && policy.statement.length > 0) {
  ...
  const handleSave = async () => {
    ...
    setLocalError(null);
    try {
      await updateMutation.mutateAsync({ accountId, bucket, json });
    } catch (err) {
      console.error(err);
  ...
  const handleClear = async () => {
    ...
    setLocalError(null);
    try {
      await deleteMutation.mutateAsync({ accountId, bucket });
      setJson("");
    } catch (err) {
      console.error(err);
      setLocalError(err instanceof Error ? err.message : String(err));
  ...
  const displayError =
    localError ||
    (fetchError ? (fetchError as Error).message : null) ||
    (updateMutation.error ? (updateMutation.error as Error).message : null) ||
