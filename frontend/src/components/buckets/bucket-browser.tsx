...
export function BucketBrowser({
  ...
  const handleDelete = (bucketName: string) => {
    setPendingDeleteBucket(bucketName);
  };
  ...
  return (
    <>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => !open && setPendingDeleteBucket(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>确定删除存储桶？</AlertDialogTitle>
            <AlertDialogDescription>
              即将删除存储桶"{pendingDeleteBucket}"，该操作不可恢复。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
