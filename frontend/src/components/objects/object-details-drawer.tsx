...
import { objects, storage } from "@wailsjs/go/models";
...
export function ObjectDetailsDrawer({ open, objectKey, onClose }: ObjectDetailsDrawerProps) {
  ...
  const [versioning, setVersioning] = useState<storage.BucketVersioningConfiguration | null>(null);
  const [lockConfig, setLockConfig] = useState<storage.ObjectLockConfiguration | null>(null);
  const [retentionState, setRetentionState] = useState<storage.ObjectRetentionState | null>(null);
  const [legalHoldState, setLegalHoldState] = useState<storage.ObjectLegalHoldState | null>(null);
  ...
  const attributesQuery = useQuery({
    queryKey: ["object-attributes", accountId, bucket, objectKey],
    queryFn: async () => {
      if (!accountId || !bucket || !objectKey) {
        throw new Error("Missing context");
      }
      return GetObjectAttributes(accountId, bucket, objectKey);
    },
    enabled: open && Boolean(accountId && bucket && objectKey),
    staleTime: 10_000,
  });
  ...
  const { updateObjectAttributes } = useObjectMutations(accountId, bucket);
  ...
  useEffect(() => {
    ...
    setAcl(attrs.acl?.canned || "private");
  ...
  const versioningQuery = useQuery({
    queryKey: ["bucket-config", accountId, bucket, "versioning:raw"],
    queryFn: async () => {
      if (!accountId || !bucket) {
        throw new Error("Missing context");
      }
      return GetBucketVersioning(accountId, bucket);
    },
    enabled: open && Boolean(accountId && bucket),
    staleTime: 30_000,
  });

  useEffect(() => {
    if (versioningQuery.data) {
      setVersioning(versioningQuery.data);
    } else if (!versioningQuery.isFetching) {
      setVersioning(null);
    }
  }, [versioningQuery.data, versioningQuery.isFetching]);
  ...
  const complianceQuery = useQuery({
    queryKey: [
      ...
      bucket,
      objectKey,
      attributesQuery.data?.object.versionId ?? "",
    ],
    queryFn: async () => {
      if (!accountId || !bucket || !objectKey) {
        throw new Error("Missing context");
      }
      const versionId = attributesQuery.data?.object.versionId ?? "";
      ...
      return { lockCfg, retention, legal };
    },
    enabled: open && Boolean(accountId && bucket && objectKey),
    staleTime: 30_000,
  });
  ...
  useEffect(() => {
    ...
    if (!payload) return;
    setLockConfig(payload.lockCfg);
    setRetentionState(payload.retention);
    setLegalHoldState(payload.legal);
    if (payload.retention?.mode) {
      setRetentionMode(payload.retention.mode);
    } else if (payload.lockCfg?.mode) {
      setRetentionMode(payload.lockCfg.mode);
    }
    setRetainUntil(
      payload.retention?.retainUntil ? formatLocalInput(payload.retention.retainUntil) : "",
    );
  }, [complianceQuery.data]);

  const loading =
    (open && attributesQuery.isPending) ||
    (open && versioningQuery.isPending) ||
    (open && complianceQuery.isPending);

  const handleSave = async () => {
    if (!accountId || !bucket || !objectKey) return;
    setSaving(true);
    try {
      await updateObjectAttributes.mutateAsync({
        bucket,
        key: objectKey,
        metadata,
        tags,
        ...
        storageClass,
        acl,
      } as any); // Cast to any or ObjectAttributesPatch if types align perfectly
