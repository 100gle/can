...
function MoveCopyDialogContent({ defaultMode, onClose }: MoveCopyDialogContentProps) {
  ...
  const currentBucket = useObjectsStore((state) => state.bucket);
  ...
  const { moveObjectsBatch, copyObject } = useObjectMutations(accountId, currentBucket);
    ...
    try {
      if (mode === "move") {
        ...
        if (requests.length === 0 && skipped.length === 0) {
          // If all skipped, we might return or show error?
          // The original code returned error if requests empty.
        }
        if (requests.length > 0) {
          // Cast requests to any if type mismatch persists, but ObjectsModels.MoveObjectRequest matched struct.
          // We need to ensure structure matches.
          // MoveObjectRequest = { sourceBucket, sourceKey, targetBucket, targetKey }
          await moveObjectsBatch.mutateAsync(requests);
        }
      } else {
        for (const key of keys) {
          const targetKey = resolveTargetKey(
      ...
      // Refresh happens via invalidation in mutation onSuccess
