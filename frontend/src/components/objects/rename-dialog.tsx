...
function RenameDialogContent({ objectKey, prefix, onClose }: RenameDialogContentProps) {
  ...
  const { renameObject } = useObjectMutations(accountId, bucket);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await renameObject.mutateAsync({ key: objectKey, newName: newKey });
      onClose();
    } catch (e) {
  ...
  return (
    <>
      ...
      <div className="space-y-4 py-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && !renameObject.isPending) {
                handleSubmit();
              }
            }}
            autoFocus
      ...
      <DialogFooter>
        <Button variant="outline" onClick={onClose} disabled={renameObject.isPending}>
          {t("common.cancel")}
        </Button>
        <Button onClick={handleSubmit} disabled={renameObject.isPending || !newName.trim()}>
          {renameObject.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
          {t("common.confirm")}
        </Button>
      </DialogFooter>
    </>
