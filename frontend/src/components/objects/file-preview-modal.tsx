...
import { storage as StorageModels } from "@wailsjs/go/models";
...
type FilePreviewModalProps = {
  ...
  accountId?: string;
  bucket?: string;
  object?: StorageModels.ObjectDescriptor;
};
...
export function FilePreviewModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (contentType) {
        // We can't use objectsStore for updates anymore.
        // Since we are inside the modal, we should use the hook.
        // But we need to lift the hook call to the component level first.
        // For now, let's skip the explicit content-type restore if it's too complex or handled by backend?
        // Actually, upload might overwrite it.
        // Let's defer this or use the hook properly.
      }
      await loadPreview(accountId, bucket, object.key);
